...
首屏的反物质维度购买按钮、计数频率按钮和新闻滚动条都属于高频可见 UI，不应只依赖 DOM 翻译。维度行与计数频率行直接在 Vue 组件中输出中文；新闻滚动条在写入 `innerHTML` 前先用规范化文本匹配中文词典，处理新闻源码中的换行、HTML 标签和动态拼接残留。回归测试需要覆盖“补至 10 个”“计数频率花费”“商店入口可见”和指定新闻 `a294` 的中文化。
