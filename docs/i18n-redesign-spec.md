...
用户持续抽查发现，单个截图修复不足以保证整体观感：部分弹窗、侧边栏、成就卡片、无限升级按钮和复制品升级按钮虽然已经中文化，但仍继承原版 `Typewriter` 或 10px 小字号，形成“中文能读但视觉不统一”的问题。

新增 `npm run audit:i18n:visual` 作为视觉一致性质量门禁：

1. 使用真实 Chrome 打开 `dist/`，分别构造新存档、无限阶段和现实阶段。
2. 遍历当前阶段所有可见 Tab/Subtab，并主动打开指数记数法、快捷键、确认选项、离线进度、自动备份和游戏玩法弹窗。
3. 检查普通中文 UI 是否仍使用 `Typewriter`，中英混排是否使用不一致字体，中文长文本行高是否低于 1.25，以及中文字号是否低于 11px。
4. 排除 CodeMirror、自动机代码块、Font Awesome、符文组件和天体符号等非普通正文 UI。
5. 报告写入 `docs/i18n-visual-audit-report.md`；候选问题数大于 0 时脚本失败。

修复策略：

1. 中文视觉层改为覆盖 `body` 下的普通 UI 文本，而不是只覆盖 `#ui/#page`，因为侧边栏和部分 Modal 会挂载在游戏主容器外。
2. 继续显式恢复图标、代码块、符文和特殊符号字体，避免全局中文字体覆盖破坏机制图形。
3. 对成就卡片中文覆盖层、快捷键说明、挑战卡片、无限升级卡片、复制品升级按钮、时间定理商店和维度重置按钮设置更适合中文的字号与行高。
4. 当前验收结果：`npm run audit:i18n:visual` 输出候选问题总数 0。
