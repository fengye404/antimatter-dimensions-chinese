...
| Tickspeed | 偶见"tick速度/计数频率" | 机制名译为“时间间隔”；按钮译为“时间间隔升级”，每秒读数译为“每秒游戏刻数” |
...
| Galaxy | 星系 | 一致 ✓ |
| Autobuyer | 自动购买者 | 一致 ✓ |
| Tickspeed | 时间间隔 / 时间间隔升级 / 每秒游戏刻数 | 已按语境修复 ✓ |
...
| 行号 | 原文 | 现有翻译 | 问题类型 | 备注 |
|------|------|----------|----------|------|
| 49 | `Tickspeed upgrades` | `计数频率 升级` | 机械直译且格式不一致 | 后续统一按语境改为“时间间隔升级”或“每秒游戏刻数” |
...
| 范围 | 修复内容 | 验证方式 | 备注 |
|------|----------|----------|------|
| 反物质维度 / 时间间隔 | 将购买模式、花费、购买数量、每秒游戏刻数说明改为源码级中文 | Playwright 首屏回归 | 修复 `Until 10`、`Buy 10`、`Tickspeed Cost` 等拆分文本 |
| 时间间隔术语 | 将“游戏刻速率”改为“每秒游戏刻数”，机制名保留“时间间隔” | Playwright 首屏回归 + 可见文本审计 | 修复主界面读数难懂的问题 |
