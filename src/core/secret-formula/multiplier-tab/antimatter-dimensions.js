...
export const AD = {
  total: {
    name: dim => {
      if (dim) return `第 ${formatInt(dim)} 反物质维度倍率`;
      if (NormalChallenge(12).isRunning) {
        if (MultiplierTabHelper.actualNC12Production().eq(0)) return "所有维度的基础反物质产量";
        return `${MultiplierTabHelper.isNC12ProducingEven() ? "偶数" : "奇数"}维度的基础反物质产量`;
      }
      return "基础反物质产量";
    },
    displayOverride: dim => {
  ...
  dimboost: {
    name: dim => (dim ? `第 ${dim} 反物质维度的维度提升` : "维度提升"),
    multValue: dim => (dim
  ...
  infinityUpgrade: {
    name: dim => (dim ? `无限升级（第 ${dim} 反物质维度）` : "无限升级"),
    multValue: dim => {
      const allMult = DC.D1.timesEffectsOf(
  ...
  infinityChallenge: {
    name: dim => (dim ? `无限挑战（第 ${dim} 反物质维度）` : "无限挑战"),
    multValue: dim => {
      const allMult = DC.D1.timesEffectsOf(
        InfinityChallenge(3),
  ...
  timeStudy: {
    name: dim => (dim ? `时间研究（第 ${dim} 反物质维度）` : "时间研究"),
    multValue: dim => {
      const allMult = DC.D1.timesEffectsOf(
        TimeStudy(91),
        TimeStudy(101),
  ...
  v: {
    name: "5 个 V 成就里程碑 - 由空间定理强化反物质维度指数",
  ...
  effectNC: {
    name: dim => (dim ? `普通挑战效果（第 ${dim} 反物质维度）` : "普通挑战效果"),
  ...
  nerfIC: {
    name: dim => (dim ? `无限挑战削弱（第 ${dim} 反物质维度）` : "无限挑战削弱"),
    multValue: dim => {
      let dimMults = Array.repeat(DC.D1, 9);
