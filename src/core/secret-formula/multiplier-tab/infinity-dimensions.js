...
export const ID = {
  total: {
    name: dim => {
      if (dim) return `第 ${dim} 无限维度倍率`;
      if (EternityChallenge(7).isRunning) return "第 7 反物质维度产量";
      return "无限能量产量";
    },
  ...
  purchase: {
    name: dim => (dim ? `已购买第 ${dim} 无限维度` : "购买数量"),
    multValue: dim => {
  ...
  highestDim: {
    ...
    displayOverride: () => {
      ...
      return `第 ${dim} 无限维度，${format(InfinityDimension(dim).amount, 2)}`;
    },
  ...
  timeStudy: {
    name: dim => (dim ? `时间研究（第 ${dim} 无限维度）` : "时间研究"),
    multValue: dim => {
  ...
  eternityChallenge: {
    name: dim => (dim ? `永恒挑战（第 ${dim} 无限维度）` : "永恒挑战"),
    multValue: dim => {
      const allMult = DC.D1.timesEffectsOf(
