...
export default {
  ...
  computed: {
    ...
    requirement() {
      if (this.id === 1) {
        return `要求：EC11 和 EC12 各完成 ${formatInt(5)} 次，且拥有
          ${formatInt(this.maxTT)}/${formatInt(TimeStudy.dilation.totalTimeTheoremRequirement)}
          总时间定理`;
      }
      if (this.id === 6) {
        const achRows = Perk.firstPerk.isBought ? "" : `，并完成 ${formatInt(13)} 行成就`;
    ...
    theoremTimeEstimate() {
      ...
      return time.gt(0) ? `${TimeSpan.fromSeconds(time.toNumber()).toStringShort()} 后拥有足够时间定理` : null;
    }
  },
  methods: {
    update() {
