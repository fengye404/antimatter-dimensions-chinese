...
export default {
  ...
  computed: {
    ...
    singleText() {
      ...
      const prefix = this.showCostTitle(this.singleCost) ? "花费：" : "";
      const suffix = this.isCostsAD ? `${this.costUnit}` : "反物质";
      return `${prefix} ${format(this.singleCost)} ${suffix}`;
    },
    until10Text() {
      if (this.isCapped) return "被无名氏破碎";
      if (this.isContinuumActive) return `连续体：${this.continuumString}`;

      const prefix = `补至 ${formatInt(10)} 个，${this.showCostTitle(this.until10Cost) ? "花费" : ""}`;
      const suffix = this.isCostsAD ? `${this.costUnit}` : "反物质";
