...
<template>
  <div class="l-pelle-celestial-tab">
    ...
    >
      需要完成 {{ formatInt(totalRows) }} 行成就，并让所有符文炼金资源达到上限，
      才能解锁反物质天神 Pelle。
      <br>
      <br>
      已完成成就行数：{{ formatInt(completedRows) }} / {{ formatInt(totalRows) }}
      <br>
      已达上限炼金资源：{{ formatInt(cappedResources) }} / {{ formatInt(totalAlchemyResources) }}
    </div>
  </div>
</template>
