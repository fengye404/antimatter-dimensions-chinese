...
export default {
  ...
  computed: {
    ...
    segmentText() {
      return this.isSegmented ? "分段速通（导入过存档）" : "单段速通（未导入存档）";
    },
    iapText() {
      return this.usedSTD ? "已使用内购项目" : "未使用内购项目";
    },
    offlineText() {
      const stateText = this.offlineProgress
        ? `<span style="color: var(--color-good)">开启</span>`
        : `<span style="color: var(--color-bad)">关闭</span>`;
      const fractionText = this.offlineFraction === 0
        ? "（未使用离线时间）"
        : `（${formatPercents(this.offlineFraction, 2)} 的时间处于离线）`;
