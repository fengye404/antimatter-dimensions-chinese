...
export default {
  ...
  computed: {
    ...
    modalTitle() {
      ...
      return this.isImporting ? "输入时间研究树" : `编辑时间研究预设“${this.name}”`;
    },
    invalidMessage() {
      ...
      return `导入字符串中包含无效的时间研究 ID：${coloredString.replaceAll("#", "").replaceAll(",", ", ")}
        <br><br>`;
    },
    truncatedInput() {
  ...
  methods: {
    ...
    savePreset() {
      if (this.inputIsValid) {
        ...
        GameUI.notify.eternity(`时间研究树“${this.name}”已编辑。`);
        this.emitClose();
      }
    },
    deletePreset() {
